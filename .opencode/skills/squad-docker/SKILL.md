...
- **A aplicação roda no host** durante o desenvolvimento (dotnet/mvn/npm direto) e só o que é infraestrutura (banco, cache, fila) roda em container — ciclo de feedback mais rápido. Containerizar a app é para quando o usuário pedir (aí entra Dockerfile multi-stage + `.dockerignore`).
